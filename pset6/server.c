...
char* indexes(const char* path)
{
    ...
    // malloc for returned string
    char* indexPhpBuffer = (char *)malloc(strlen(path) + strlen("index.php") + 1);  
    // copy path into returned string
    strcpy(indexPhpBuffer, path);
    // concatenate test string with "/index.php" string
    strcat(indexPhpBuffer, "index.php");
    strcat(indexPhpBuffer, "\0");
    // access function with F_OK tests for the existence of the file.
    if (access(indexPhpBuffer, F_OK) != -1)
    {
        return indexPhpBuffer;
    }
    
    free(indexPhpBuffer);
    
    // if there is no index.php in the folder, check for index.html 
    // malloc for returned string
    char* indexHtmlBuffer = (char *)malloc(strlen(path) + strlen("index.html") + 1);   
    // copy path into returned string
    strcpy(indexHtmlBuffer, path);
    // concatenate test string with "/index.html" string
    strcat(indexHtmlBuffer, "index.html");
    strcat(indexHtmlBuffer, "\0");
    // access function with F_OK tests for the existence of the file.
    if (access(indexHtmlBuffer, F_OK)!= -1)
    {
        return indexHtmlBuffer;
    }
    
    
    free(indexHtmlBuffer);
    // return if there is no such files 
    return NULL;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    // byte counter for file length
    size_t countByte = 0;
    // read input file to buffer with blocks of 1 byte 1 at time
	while (fread(buffer,1,1, file) == 1)
	{
        // realloc memory + 1 byte each time in the loop
        *content = (BYTE *)realloc(*content, sizeof(char)+countByte);
        if(*content == NULL)
        {
            return false;
        } 
        // copy date from temp buffer to content
        memcpy(*content + countByte, buffer, 1);
        countByte++;
	}
    
    // return the adress of the loaded file
    //*content = buffer;
...
const char* lookup(const char* path)
{
    ...
    //The strrchr function searches string(path) for the last occurrence of char '.' 
    s = strrchr (path, '.');
    ...
    */
    ...
    if (s != NULL)
    {
        /* compare 4 chars in string (s) with file extensions, 
        if zero, that means they are identical*/
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // split parts - tokens
    char* methodString = strtok(request_line, " ");
    char* stringRequestTarget = strtok(NULL, " ");
    char* httpVersionString = strtok(NULL, " ");
    
    // Error checking
    if(methodString == NULL || stringRequestTarget == NULL || httpVersionString == NULL)
    {
        error(400);
        return false;
    }
    ...
    // check HTTP/1.1. if HTTP-version is not HTTP/1.1, respond to the browser with 505 HTTP Version Not Supported and return false
    if (strcmp(httpVersionString, "HTTP/1.1\r\n") != 0)
    {
        error(505);
        return false;
    }
    ...
    // a request-target may not contain "
    for(int i = 0; i < strlen(stringRequestTarget); i++)
    {
        if (stringRequestTarget[i] == '"')
        {
            error(400);
            return false;
        }
    ...
    /*
    whereby absolute-path (which will not contain ?) 
    must start with / and might optionally be followed by a ? 
    followed by a query, which may not contain ".
    */
    char absolutePathBuffer[LimitRequestLine + 1];
    strcpy(absolutePathBuffer, abs_path);
    // absolute path return
    int i = 0;
    for(; i < LimitRequestLine && stringRequestTarget[i] != '?' && stringRequestTarget[i] != ' '; i++)
    {
        // case insensitive filename
        absolutePathBuffer[i] = stringRequestTarget[i];
    }
    absolutePathBuffer[i] = '\0';
    strcpy(abs_path, absolutePathBuffer);
    
    

    char queryBuffer[LimitRequestLine + 1];
    strcpy(queryBuffer, query);
    // query return
    char *question;
    int index;
    int j;
    question = strchr(stringRequestTarget, '?');
    if (question != NULL)
    {
        for(index = (int)(question - stringRequestTarget)+1, j = 0; index < LimitRequestLine+1 && stringRequestTarget[index] != ' '; index++, j++)
        {
            queryBuffer[j] = stringRequestTarget[index];
        }
        queryBuffer[j] = '\0';
    } 
    else 
